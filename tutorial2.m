%% Delta Robot Project - Tutorial Reference Script
% NOTE: This simulates a 2-link serial planar arm, NOT a Delta robot.
% Use this to learn the rigidBodyTree workflow, then apply the same
% structure to your actual Delta model.
...
view(2); ax = gca; ax.Projection = 'orthographic';
title('After adding Link 1'); axis([-0.1 0.7 -0.3 0.5]);
...
show(robot, homeConfiguration(robot));
view(2); ax = gca; ax.Projection = 'orthographic';
title('After adding Link 2'); axis([-0.1 0.7 -0.3 0.5]);
...
show(robot, q_home); view(2); ax = gca; ax.Projection = 'orthographic';
title('Complete Robot in Home Configuration'); axis([-0.1 0.7 -0.3 0.5]);
...
show(robot, q_test); view(2); ax = gca; ax.Projection = 'orthographic';
...
show(robot, homeConfiguration(robot)); view(2); ax = gca; ax.Projection = 'orthographic';
hold on;
...
theta_path = t * (2*pi / t(end));
points = center + radius * [cos(theta_path), sin(theta_path), zeros(size(theta_path))];
...
if min(distances) >= innerR, fprintf('OK\n'); else, fprintf('WARNING: some points inside dead zone!\n'); end
fprintf('Max distance from base: %.4f m  (outer boundary: %.2f m) — ', max(distances), outerR);
if max(distances) <= outerR, fprintf('OK\n'); else, fprintf('WARNING: some points outside reach!\n'); end
...
legend('Location','best');
title('Desired Trajectory Inside Workspace'); axis equal; hold off;
...
show(robot, qSol1); view(2); ax = gca; ax.Projection = 'orthographic';
hold on;
...
legend('Location','best');
title('IK Solution for First Waypoint'); axis([-0.1 0.7 -0.3 0.5]); hold off;
...
show(robot, qs(1,:)'); view(2); ax = gca; ax.Projection = 'orthographic';
hold on;
